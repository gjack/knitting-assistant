...
METADATA_SYSTEM_PROMPT = """You are a knitting pattern parser. Extract the following fields as JSON: title (string), sizes (list of strings), gauge (string), needles (list of strings), yarn (string or list), abbreviations (list of {symbol: string, meaning: string}), sections (list of section title strings). If a field is not present in the pattern, use null or an empty list. Return only valid JSON."""
